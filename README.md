...
HTTP服务器，即Web服务器，也是使用套接字。HTTP属于无状态的协议，即服务器不维护客户端状态，在响应客户端请求后就会立即断开连接，即使同一客户端再次发送请求，服务器也不做辨认，而是以相同方式处理。有必要保持连接时，Web编程中会使用Cookie和Session技术。

### 请求和响应

HTTP请求消息的结构分为请求行、消息头和消息体三个部分。请求行含有请求方式和目的等信息，典型的请求方法有GET和POST，分别用于获取和发送数据。例如如下的请求行：
```
GET/index.html HTTP/1.1
```
以上请求行表示要获取index.html文件，并使用1.1版本的HTTP协议进行通信。服务器从请求的第一行获取到以上信息，之后的消息头部分包括浏览器信息、用户认证信息等，最后的消息体部分含有需要传输的数据，消息体和消息头之间用空行分隔。

响应消息也是由三部分构成，包括状态行、消息头和消息体。标准的状态行内容如下：
```
HTTP/1.1 200 OK
```
以上状态行传递的信息是，使用HTTP协议版本1.1响应，请求已经正确处理。其他返回状态还可能有404 Not Found表示请求的文件不存在，400 Bad Request表示请求方式错误。消息头中包含服务器端的名称，传输数据类型，数据长度信息等。同样，在消息头和消息体之间会有一个空行。

### Linux下的Web服务器

以下使用小程序代号73，演示Linux系统下创建Web服务器。
```
$ gcc 73_webserv_linux.c -o linux_web
Lings-MacBook-Pro:Socket ljiang$ ./linux_web 9190
